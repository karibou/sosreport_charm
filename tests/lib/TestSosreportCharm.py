...
class TestSosreportCharm(unittest.TestCase):
    ...
    def run_test(self):
        ...
        try:
            ...
            self.deployment.log.debug("starting deployment for %d seconds" % self.seconds)
        except amulet.helpers.TimeoutError:
            message = 'The environment did not setup in %d seconds.' % self.seconds
        ...
        self.service_stat = self.deployment.sentry['sosreport'][0].info['agent-state']
        self.file_stat = self.deployment.sentry['sosreport'][0].file('/usr/bin/sosreport')
        self.deployment.log.debug("Testing if the service is started")
        self.assertEqual(self.service_stat, 'started')
        self.deployment.log.debug("Testing if the sosreport executable is present")
